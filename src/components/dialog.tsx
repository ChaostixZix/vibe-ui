...
>(({ className, open, onOpenChange, children, uncloseable, ...props }, ref) => {
  // Basic keyboard handling for Escape key
  React.useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape' && open && onOpenChange && !uncloseable) {
        onOpenChange(false);
      }
    };
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-start justify-center p-4 overflow-y-auto">
      ...
      <div
        ref={ref}
        className={cn(
          'relative z-[9999] grid w-full max-w-lg gap-4 bg-background p-6 shadow-lg duration-200 rounded-none my-8',
          className
        )}
        {...props}
      >
